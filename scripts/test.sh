...
printf "🧪 Iniciando testes dos dotfiles..."

# Função para verificar symlinks
check_symlinks() {
    printf "🔗 Verificando symlinks..."
    while read -r source_path target_path; do
        ...
        if [ ! -L "$target_full" ]; then
            printf "❌ Symlink não encontrado: $target_full"
            exit 1
        fi
        if [ "$(readlink -f "$target_full")" != "$source_full" ]; then
            printf "❌ Symlink incorreto: $target_full -> $(readlink -f "$target_full") (esperado: $source_full)"
            exit 1
        fi
        printf "✅ Symlink OK: $target_full"
    done < "$CONFIG_FILE"
}

# Função para testar carregamento do Zsh
check_zsh() {
    printf "🐚 Testando carregamento do Zsh..."
    if ! zsh -c 'source ~/.zshrc && printf "Zsh carregado com sucesso"' >/dev/null 2>&1; then
        printf "❌ Falha ao carregar Zsh"
        exit 1
    fi
    printf "✅ Zsh OK"
}
...
check_plugins() {
    printf "🧩 Verificando plugins..."
    plugins=("zsh-autosuggestions" "zsh-syntax-highlighting")
    for plugin in "${plugins[@]}"; do
        if [ ! -d "$ZSH_CUSTOM/plugins/$plugin" ]; then
            printf "❌ Plugin não encontrado: $plugin"
            exit 1
        fi
        printf "✅ Plugin OK: $plugin"
    done
}

# Teste de existência e integridade do arquivo .p10k.zsh
check_p10k_file() {
    printf "🎨 Verificando arquivo .p10k.zsh..."
    local p10k_file="$DOTFILES_DIR/zsh/.p10k.zsh"
    if [ ! -f "$p10k_file" ]; then
        printf "❌ Arquivo .p10k.zsh não encontrado!"
        exit 1
    fi
    ...
    for theme in .p10k-clean.zsh .p10k-darkest.zsh .p10k-rainbow.zsh; do
        if cmp -s "$DOTFILES_DIR/zsh/$theme" "$p10k_file"; then
            valid=1
            printf "✅ .p10k.zsh corresponde ao tema: $theme"
            break
        fi
    done
    if [ $valid -eq 0 ]; then
        printf "❌ .p10k.zsh não corresponde a nenhum tema conhecido!"
        exit 1
    fi
}
...
test_p10k_fallback() {
    printf "🧪 Testando fallback automático do Powerlevel10k..."
    local p10k_file="$DOTFILES_DIR/zsh/.p10k.zsh"
    rm -f "$p10k_file"
    "$DOTFILES_DIR/scripts/install.sh" < /dev/null
    if [ ! -f "$p10k_file" ]; then
        printf "❌ Fallback do Powerlevel10k falhou: .p10k.zsh não criado!"
        exit 1
    fi
    printf "✅ Fallback automático OK"
}

# Teste de seleção de tema
test_theme_selection() {
    printf "🧪 Testando seleção de tema..."
    for choice in 1 2 3; do
        ...
        if cmp -s "$theme_file" "$p10k_file"; then
            printf "✅ Seleção de tema $choice OK"
        else
            printf "❌ Seleção de tema $choice falhou!"
            exit 1
        fi
    done
}
